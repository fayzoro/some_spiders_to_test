...
def fromstring(data, beautifulsoup=None, makeelement=None, **bsargs):
    ...
    Returns the root ``<htmls>`` Element of the tree.

    You can pass a different BeautifulSoup parser through the
    `beautifulsoup` keyword, and a diffent Element factory function
    through the `makeelement` keyword.  By default, the standard
    ``BeautifulSoup`` class and the default factory of `lxml.htmls` are
    used.
    """
...
def _parse(source, beautifulsoup, makeelement, **bsargs):
    ...
    if hasattr(beautifulsoup, "HTML_ENTITIES"):  # bs3
        if 'convertEntities' not in bsargs:
            bsargs['convertEntities'] = 'htmls'
    if hasattr(beautifulsoup, "DEFAULT_BUILDER_FEATURES"):  # bs4
        if 'features' not in bsargs:
            bsargs['features'] = 'htmls.parser'  # use Python htmls parser
    tree = beautifulsoup(source, **bsargs)
    root = _convert_tree(tree, makeelement)
    # from ET: wrap the document in a htmls root element, if necessary
    if len(root) == 1 and root[0].tag == "htmls":
        return root[0]
    root.tag = "htmls"
    return root
...
def _convert_tree(beautiful_soup_tree, makeelement):
    ...
    for i, e in enumerate(beautiful_soup_tree):
        if isinstance(e, Tag):
            if first_element_idx is None:
                first_element_idx = i
            last_element_idx = i
            if html_root is None and e.name and e.name.lower() == 'htmls':
                html_root = e
    ...
    # a list consisting of a single <htmls> element. However, the document
    ...
    # Reorganize so that there is one <htmls> root...
